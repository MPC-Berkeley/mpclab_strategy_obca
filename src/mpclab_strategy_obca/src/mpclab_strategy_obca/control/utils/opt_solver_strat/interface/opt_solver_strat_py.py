...
a Python wrapper for a fast solver generated by FORCES PRO v4.0.0
   ...
   OUTPUT = opt_solver_strat_py.opt_solver_strat_solve(PARAMS) solves a multistage problem
   subject to the parameters supplied in the following dictionary:
       ...
       PARAMS['all_parameters'] - column vector of length 567
   ...
   [OUTPUT, EXITFLAG] = opt_solver_strat_py.opt_solver_strat_solve(PARAMS) returns additionally
   ...
   [OUTPUT, EXITFLAG, INFO] = opt_solver_strat_py.opt_solver_strat_solve(PARAMS) returns
...
#_lib = ctypes.CDLL(os.path.join(os.getcwd(),'opt_solver_strat/lib/opt_solver_strat.so'))
try:
	_lib = ctypes.CDLL(os.path.join(os.path.dirname(os.path.abspath(__file__)),'../lib/opt_solver_strat_withModel.so'))
	csolver = getattr(_lib,'opt_solver_strat_solve')
except:
	_lib = ctypes.CDLL(os.path.join(os.path.dirname(os.path.abspath(__file__)),'../lib/libopt_solver_strat_withModel.so'))
	csolver = getattr(_lib,'opt_solver_strat_solve')

class opt_solver_strat_params_ctypes(ctypes.Structure):
#	@classmethod
#	def from_param(self):
#		return self
	_fields_ = [('x0', ctypes.c_double * 542),
('xinit', ctypes.c_double * 6),
('all_parameters', ctypes.c_double * 567),
]

opt_solver_strat_params = {'x0' : np.array([]),
'xinit' : np.array([]),
...
opt_solver_strat_params_types = {'x0' : np.float64,
'xinit' : np.float64,
'all_parameters' : np.float64,
}

class opt_solver_strat_outputs_ctypes(ctypes.Structure):
#	@classmethod
#	def from_param(self):
#		return self
...
opt_solver_strat_outputs = {'x01' : np.array([]),
'x02' : np.array([]),
'x03' : np.array([]),
'x04' : np.array([]),
'x05' : np.array([]),
...
class opt_solver_strat_info(ctypes.Structure):
#	@classmethod
#	def from_param(self):
#		return self
...
csolver.argtypes = ( ctypes.POINTER(opt_solver_strat_params_ctypes), ctypes.POINTER(opt_solver_strat_outputs_ctypes), ctypes.POINTER(opt_solver_strat_info), ctypes.POINTER(FILE))
csolver.restype = ctypes.c_int

def opt_solver_strat_solve(params_arg):
	'''
a Python wrapper for a fast solver generated by FORCES PRO v4.0.0
 ...
 See also COPYING
	...
	# convert parameters
	params_py = opt_solver_strat_params_ctypes()
	for par in params_arg:
		try:
			#setattr(params_py, par, npct.as_ctypes(np.reshape(params_arg[par],np.size(params_arg[par]),order='A')))
			params_arg[par] = np.require(params_arg[par], dtype=opt_solver_strat_params_types[par], requirements='F')
	...
	outputs_py = opt_solver_strat_outputs_ctypes()
	info_py = opt_solver_strat_info()
	if sys.version_info.major == 2:
		...
		try:
			PyFile_AsFile.restype = ctypes.POINTER(FILE)
			exitflag = _lib.opt_solver_strat_solve( params_py, ctypes.byref(outputs_py), ctypes.byref(info_py), PyFile_AsFile(fp) , _lib.opt_solver_strat_casadi2forces )
		...
		try:
			if sys.platform.startswith('win'):
				exitflag = _lib.opt_solver_strat_solve( params_py, ctypes.byref(outputs_py), ctypes.byref(info_py), None , _lib.opt_solver_strat_casadi2forces)
			else:
				exitflag = _lib.opt_solver_strat_solve( params_py, ctypes.byref(outputs_py), ctypes.byref(info_py), fp , _lib.opt_solver_strat_casadi2forces)
			libc.fclose(fp)
	...
	# convert outputs
	for out in opt_solver_strat_outputs:
		opt_solver_strat_outputs[out] = npct.as_array(getattr(outputs_py,out))

	return opt_solver_strat_outputs,int(exitflag),info_py

solve = opt_solver_strat_solve
