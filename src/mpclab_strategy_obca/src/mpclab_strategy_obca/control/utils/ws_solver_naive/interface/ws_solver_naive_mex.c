...
#include "mex.h"
#include "math.h"
#include "../include/ws_solver_naive.h"
...
extern void ws_solver_naive_casadi2forces(ws_solver_naive_float *x, ws_solver_naive_float *y, ws_solver_naive_float *l, ws_solver_naive_float *p, ws_solver_naive_float *f, ws_solver_naive_float *nabla_f, ws_solver_naive_float *c, ws_solver_naive_float *nabla_c, ws_solver_naive_float *h, ws_solver_naive_float *nabla_h, ws_solver_naive_float *hess, solver_int32_default stage, solver_int32_default iteration);
ws_solver_naive_extfunc pt2function_ws_solver_naive = &ws_solver_naive_casadi2forces;


/* Some memory for mex-function */
static ws_solver_naive_params params;
static ws_solver_naive_output output;
static ws_solver_naive_info info;
...
void mexFunction( solver_int32_default nlhs, mxArray *plhs[], solver_int32_default nrhs, const mxArray *prhs[] )  
{
    ...
    if (nrhs != 1) 
	{
        mexErrMsgTxt("This function requires exactly 1 input: PARAMS struct.\nType 'help ws_solver_naive_mex' for details.");
    }    
	if (nlhs > 3) 
	{
    ...
    if( mxGetM(par) != 441 || mxGetN(par) != 1 ) 
	{
    mexErrMsgTxt("PARAMS.x0 must be of size [441 x 1]");
    }
#endif	 
	if ( (mxGetN(par) != 0) && (mxGetM(par) != 0) )
	{
		copyMArrayToC_double(mxGetPr(par), params.x0,441);
	...
	#if SET_PRINTLEVEL_ws_solver_naive > 0
	...
	/* call solver */
	exitflag = ws_solver_naive_solve(&params, &output, &info, fp, pt2function_ws_solver_naive);
	...
	copyCArrayToM_double( output.x01, mxGetPr(outvar), 21);
	mxSetField(plhs[0], 0, "x01", outvar);

	outvar = mxCreateDoubleMatrix(21, 1, mxREAL);
	copyCArrayToM_double( output.x02, mxGetPr(outvar), 21);
	mxSetField(plhs[0], 0, "x02", outvar);

	outvar = mxCreateDoubleMatrix(21, 1, mxREAL);
	copyCArrayToM_double( output.x03, mxGetPr(outvar), 21);
	mxSetField(plhs[0], 0, "x03", outvar);

	outvar = mxCreateDoubleMatrix(21, 1, mxREAL);
	copyCArrayToM_double( output.x04, mxGetPr(outvar), 21);
	mxSetField(plhs[0], 0, "x04", outvar);

	outvar = mxCreateDoubleMatrix(21, 1, mxREAL);
	copyCArrayToM_double( output.x05, mxGetPr(outvar), 21);
	mxSetField(plhs[0], 0, "x05", outvar);

	outvar = mxCreateDoubleMatrix(21, 1, mxREAL);
	copyCArrayToM_double( output.x06, mxGetPr(outvar), 21);
	mxSetField(plhs[0], 0, "x06", outvar);

	outvar = mxCreateDoubleMatrix(21, 1, mxREAL);
	copyCArrayToM_double( output.x07, mxGetPr(outvar), 21);
	mxSetField(plhs[0], 0, "x07", outvar);

	outvar = mxCreateDoubleMatrix(21, 1, mxREAL);
	copyCArrayToM_double( output.x08, mxGetPr(outvar), 21);
	mxSetField(plhs[0], 0, "x08", outvar);

	outvar = mxCreateDoubleMatrix(21, 1, mxREAL);
	copyCArrayToM_double( output.x09, mxGetPr(outvar), 21);
	mxSetField(plhs[0], 0, "x09", outvar);

	outvar = mxCreateDoubleMatrix(21, 1, mxREAL);
	copyCArrayToM_double( output.x10, mxGetPr(outvar), 21);
	mxSetField(plhs[0], 0, "x10", outvar);

	outvar = mxCreateDoubleMatrix(21, 1, mxREAL);
	copyCArrayToM_double( output.x11, mxGetPr(outvar), 21);
	mxSetField(plhs[0], 0, "x11", outvar);

	outvar = mxCreateDoubleMatrix(21, 1, mxREAL);
	copyCArrayToM_double( output.x12, mxGetPr(outvar), 21);
	mxSetField(plhs[0], 0, "x12", outvar);

	outvar = mxCreateDoubleMatrix(21, 1, mxREAL);
	copyCArrayToM_double( output.x13, mxGetPr(outvar), 21);
	mxSetField(plhs[0], 0, "x13", outvar);

	outvar = mxCreateDoubleMatrix(21, 1, mxREAL);
	copyCArrayToM_double( output.x14, mxGetPr(outvar), 21);
	mxSetField(plhs[0], 0, "x14", outvar);

	outvar = mxCreateDoubleMatrix(21, 1, mxREAL);
	copyCArrayToM_double( output.x15, mxGetPr(outvar), 21);
	mxSetField(plhs[0], 0, "x15", outvar);

	outvar = mxCreateDoubleMatrix(21, 1, mxREAL);
	copyCArrayToM_double( output.x16, mxGetPr(outvar), 21);
	mxSetField(plhs[0], 0, "x16", outvar);

	outvar = mxCreateDoubleMatrix(21, 1, mxREAL);
	copyCArrayToM_double( output.x17, mxGetPr(outvar), 21);
	mxSetField(plhs[0], 0, "x17", outvar);

	outvar = mxCreateDoubleMatrix(21, 1, mxREAL);
	copyCArrayToM_double( output.x18, mxGetPr(outvar), 21);
	mxSetField(plhs[0], 0, "x18", outvar);

	outvar = mxCreateDoubleMatrix(21, 1, mxREAL);
	copyCArrayToM_double( output.x19, mxGetPr(outvar), 21);
	mxSetField(plhs[0], 0, "x19", outvar);

	outvar = mxCreateDoubleMatrix(21, 1, mxREAL);
	copyCArrayToM_double( output.x20, mxGetPr(outvar), 21);
	mxSetField(plhs[0], 0, "x20", outvar);

	outvar = mxCreateDoubleMatrix(21, 1, mxREAL);
	copyCArrayToM_double( output.x21, mxGetPr(outvar), 21);
