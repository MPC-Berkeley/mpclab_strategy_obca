...
# It is still buggy to generate solvers directly with Python. So in this code we are loading the MATLAB-generated solver instead
...
class NaiveOBCAParameterizedController(abstractController):
	...
	def initialize(self, regen=False):
		...
		if regen:
			...
			# ==== The hacky way to call the generated solver from MATLAB client
			self.ws_solver = MatlabFPSolver(ws_solver_naive_solve)
			self.opt_solver = MatlabFPSolver(opt_solver_naive_param_solve)
